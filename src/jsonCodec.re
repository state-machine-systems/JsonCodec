...
let null: codec unit = (const Json.null, decode_raw_null >>> map (const ()));
...
let decode_array_elements
    (decode: json_decoder 'a)
    (elements: array Json.t)
    :decoder_result (array 'a) => {
...
let array ((enc, dec): codec 'a) :codec (Array.t 'a) => {
  ...
  let decode json => decode_raw_array json >>= decode_array_elements dec;
  (encode, decode)
};
...
let object1 ((enc1, dec1): field_codec 'a) :codec 'a => {
  ...
  let decode json => decode_raw_object json >>= (fun dict => dec1 dict);
  (encode, decode)
};

let object2 ((enc1, dec1): field_codec 'a) ((enc2, dec2): field_codec 'b) :codec ('a, 'b) => {
  let encode (v1, v2) => Json.object_ (build_dict [enc1 v1, enc2 v2]);
  let decode json =>
    decode_raw_object json >>= (
...
let object3
    ((enc1, dec1): field_codec 'a)
    ((enc2, dec2): field_codec 'b)
    ((enc3, dec3): field_codec 'c)
    :codec ('a, 'b, 'c) => {
  let encode (v1, v2, v3) => Json.object_ (build_dict [enc1 v1, enc2 v2, enc3 v3]);
  let decode json =>
    decode_raw_object json >>= (
      fun dict =>
        dec1 dict >>= (
...
let object4
    ...
    :codec ('a, 'b, 'c, 'd) => {
  let encode (v1, v2, v3, v4) => Json.object_ (build_dict [enc1 v1, enc2 v2, enc3 v3, enc4 v4]);
  let decode json =>
    decode_raw_object json >>= (
      fun dict =>
        dec1 dict >>= (
...
let object8
  ...
  let encode (v1, v2, v3, v4, v5, v6, v7, v8) =>
    Json.object_ (
      build_dict [enc1 v1, enc2 v2, enc3 v3, enc4 v4, enc5 v5, enc6 v6, enc7 v7, enc8 v8]
    );
  let decode json =>
    decode_raw_object json >>= (
      fun dict =>
...
let object9
  ...
  let encode (v1, v2, v3, v4, v5, v6, v7, v8, v9) =>
    Json.object_ (
      build_dict [enc1 v1, enc2 v2, enc3 v3, enc4 v4, enc5 v5, enc6 v6, enc7 v7, enc8 v8, enc9 v9]
    );
  let decode json =>
    decode_raw_object json >>= (
      fun dict =>
